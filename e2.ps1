...
function Find-EpgEvent{
 ...
 if(-not $event.PSObject.Properties.Match("id").Count){
  Write-Log "Matching EPG event has no eventId." "WARN"
  return $null
 }

 if(-not $event.PSObject.Properties.Match("sref").Count){
  $event|Add-Member -NotePropertyName sref -NotePropertyValue $script:Config.Channels[$channel_name]
 }

 Write-Log ("EPG match: '{0}' eventId={1} sref={2} start={3}" -f $event.title,$event.id,$event.sref,$event.begin_timestamp) "DEBUG"
...
function New-RecordingByEventId{
 ...
 Write-Log ("Creating DVR timer eventId={0} sRef={1}" -f $eventId,$sRef) "DEBUG"

 $res=Invoke-e2Api -Path $script:Api.CreateById -Query @{sRef=$sRef;eventid=$eventId;afterevent=$script:AfterEvent}
...
try{
 ...
 switch($Action){
  "add"{
   ...
   $event=Find-EpgEvent -searchPhrase $searchPhrase -startTime $start_unix

   if($null -ne $event){

     if([string]::IsNullOrWhiteSpace($event.sref)){
       $event.sref=$sRef
     }

     Write-Log ("EPG event found: id={0} title='{1}'" -f $event.id,$event.title) "DEBUG"

     #kein Padding berechnen (macht Enigma2).
     $result=New-RecordingByEventId -sRef $event.sref -eventId $event.id

     if($result.PSObject.Properties.Match("message").Count){
       Write-Log $result.message "INFO"
     }

     Write-Log ("DVR added via timeraddbyeventid:: '{0}' EventID={1}" -f $title,$event.id) "INFO"
   }else{
